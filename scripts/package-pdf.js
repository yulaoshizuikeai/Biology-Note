...
async function packagePdfs() {
  ...
  const pdfFiles = await fg(["**/*.pdf"], {
    cwd: pdfRepoDir,
    ignore: ["Biology-Note-Complete.pdf", "Biology-Note-All-PDFs.zip"],
  });
  ...
  if (fs.existsSync(completePdfPath)) {
    fs.copyFileSync(completePdfPath, path.join(distPdfDir, "Biology-Note-Complete.pdf"));
  }
  if (fs.existsSync(zipPath)) {
    fs.copyFileSync(zipPath, path.join(distPdfDir, "Biology-Note-All-PDFs.zip"));
  }
