<template>
  <div class="interactive-card">
    <div class="card-header">
      ...
      <p class="card-desc">下拉切换 7 大经典变式，4×4 棋盘 16 格平滑聚类变色合并，一秒推导表型比与测交比。</p>
    </div>
    ...
    <div class="grid-container">
      <div class="grid-4x4">
        <div v-for="(cell, i) in gridCells" :key="i" class="punnett-cell" :class="getCellColorClass(cell)">
...
const selectedVariation = ref('classic');
...
.ctrl-bar { margin: 16px 0; display: flex; align-items: center; gap: 12px; flex-wrap: wrap; }
.select-box {
  padding: 8px 14px; border-radius: 8px; border: 1px solid var(--vp-c-divider); background: var(--vp-c-bg); color: var(--vp-c-text-1); font-weight: 600; font-size: 13px;
}
.grid-container { display: grid; grid-template-columns: repeat(auto-fit, minmax(280px, 1fr)); gap: 20px; }
.grid-4x4 { display: grid; grid-template-columns: repeat(4, 1fr); gap: 6px; max-width: 320px; }
.punnett-cell {
...
.c-purple { background: #f3e8ff; color: #6b21a8; border-color: #d8b4fe; }
.c-gray { background: #f1f5f9; color: #475569; border-color: #cbd5e1; }
.summary-panel { padding: 16px; border-radius: 8px; background: var(--vp-c-bg); border: 1px solid var(--vp-c-divider); display: flex; flex-direction: column; justify-content: space-around; }
.result-row { font-size: 14px; margin-bottom: 8px; display: flex; justify-content: space-between; }
.text-brand { color: var(--vp-c-brand-1); font-size: 18px; font-family: monospace; }
.text-amber { color: #d97706; font-size: 16px; font-family: monospace; }
.result-desc strong { font-size: 12.5px; color: var(--vp-c-text-1); }
.result-desc p { font-size: 12px; color: var(--vp-c-text-2); line-height: 1.5; margin: 4px 0 0; }
</style>