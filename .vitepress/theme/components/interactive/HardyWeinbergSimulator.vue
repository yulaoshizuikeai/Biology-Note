<template>
  <div class="interactive-card">
    <div class="card-header">
      ...
      <p class="card-desc">
        拖动显性基因频率 p，即时输出隐性基因频率 q 及三种基因型频率，直观对比伴 X
        遗传男女发病率悬殊差距。
      </p>
    </div>
    ...
    <div class="slider-box">
      <label
        >显性基因频率 (p): <span class="val">{{ p.toFixed(2) }}</span> ➔ 隐性基因频率 (q):
        <span class="val">{{ q.toFixed(2) }}</span></label
      >
    ...
    <div class="results-grid">
      <div class="freq-card">
        ...
        <div class="freq-bar">
          <div class="bar-fill bg-blue" :style="{ width: p2 * 100 + '%' }"></div>
        </div>
      ...
      <div class="freq-card">
        ...
        <div class="freq-bar">
          <div class="bar-fill bg-green" :style="{ width: twoPq * 100 + '%' }"></div>
        </div>
      ...
      <div class="freq-card">
        ...
        <div class="freq-bar">
          <div class="bar-fill bg-amber" :style="{ width: q2 * 100 + '%' }"></div>
        </div>
    ...
    <div class="x-linked-box">
      <div class="x-title">
        ★ 高考压轴对比：若该隐性基因为伴 X 染色体致病基因（如红绿色盲/血友病）：
      </div>
      <div class="x-stats">
        <div class="x-item">
          男性发病率 (XᵃY = q)：<strong>{{ (q * 100).toFixed(2) }}%</strong>
        </div>
        <div class="x-item">
          女性发病率 (XᵃXᵃ = q²)：<strong>{{ (q2 * 100).toFixed(2) }}%</strong>
        </div>
        <div class="x-item ratio">
          男女发病倍数比：<strong>{{ (1 / q).toFixed(1) }} 倍！</strong>
        </div>
      </div>
      <p class="x-note">
        结论：伴 X
        隐性遗传病在男性中的发病率（q）远高于女性（q²），随着基因频率越低，男女发病差距越呈几何级倍增！
      </p>
    </div>
  </div>
</template>
