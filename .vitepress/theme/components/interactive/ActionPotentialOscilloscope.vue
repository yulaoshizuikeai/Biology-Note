<template>
  <div class="interactive-card">
    <div class="card-header">
      ...
      <p class="card-desc">调节刺激强度与胞外 Na⁺ 浓度，观察离子通道启闭、去极化/复极化波形与示波器指针偏转。</p>
    </div>

    <div class="controls-deck">
      <button :class="['stimulate-btn', { firing: isFiring }]" :disabled="isFiring" @click="triggerStimulation">
        ⚡ 施加外界刺激 (激发冲动)
      </button>

      <div class="ctrl-group">
        <label>胞外 Na⁺ 浓度水平: <span class="val">{{ naLevel }}%</span></label>
        <div class="pill-group">
          <button :class="['pill-btn', { active: naLevel === 70 }]" @click="naLevel = 70">低钠 (70%)</button>
          <button :class="['pill-btn', { active: naLevel === 100 }]" @click="naLevel = 100">正常 (100%)</button>
          <button :class="['pill-btn', { active: naLevel === 130 }]" @click="naLevel = 130">高钠 (130%)</button>
        </div>
      </div>
    </div>
...
const phaseName = computed(() => {
  switch (phaseIndex.value) {
    case 0: return '静息状态 (Resting)';
    case 1: return '去极化阶段 (Depolarization)';
    case 2: return '动作电位峰值 (Peak Action)';
    case 3: return '复极化与超极化 (Repolarization)';
    case 4: return '静息电位恢复 (Recovery)';
    default: return '静息状态';
  }
});
...
const phaseDescription = computed(() => {
  switch (phaseIndex.value) {
    case 0: return '膜对 K⁺ 通透性高，K⁺ 经通道蛋白外流，膜电位维持在【外正内负】(-70mV)。';
    case 1: return '受阈刺激激发，电压门控 Na⁺ 通道瞬间大开，Na⁺ 顺浓度差快速内流，膜电位倒转！';
    case 2: return `膜内电位冲至峰值 (+${peakVoltage.value}mV)，电表发生第一次偏转！胞外高钠峰值更高，低钠峰值降低。`;
    case 3: return 'Na⁺ 通道失活关闭，电压门控 K⁺ 通道开放，K⁺ 快速外流，膜电位回归负值，电表发生第二次反向偏转！';
    case 4: return 'Na⁺-K⁺ 泵主动运输消耗 ATP，逆浓度泵出 3 个 Na⁺、泵入 2 个 K⁺，彻底恢复原始离子浓度梯度的静态平衡。';
    default: return '';
  }
});
...
const phaseTransportType = computed(() => {
  switch (phaseIndex.value) {
    case 0: return 'K⁺ 外流 ➔ 协助扩散 (不耗能)';
    case 1: return 'Na⁺ 内流 ➔ 协助扩散 (不耗能)';
    case 2: return '通道关闭失活';
    case 3: return 'K⁺ 外流 ➔ 协助扩散 (不耗能)';
    case 4: return 'Na⁺-K⁺ 泵转运 ➔ 主动运输 (消耗 ATP)';
    default: return '协助扩散';
  }
});
...
.controls-deck { display: flex; justify-content: space-between; align-items: center; margin: 16px 0; flex-wrap: wrap; gap: 12px; }
.stimulate-btn {
  padding: 10px 20px; border-radius: 8px; background: #ef4444; color: #fff; font-weight: 700; font-size: 13px; border: none; cursor: pointer; transition: all 0.2s;
}
.stimulate-btn:disabled { opacity: 0.6; cursor: not-allowed; }
.pill-group { display: flex; gap: 6px; margin-top: 4px; }
.pill-btn { padding: 5px 12px; border-radius: 6px; font-size: 12px; font-weight: 600; border: 1px solid var(--vp-c-divider); background: var(--vp-c-bg); color: var(--vp-c-text-2); cursor: pointer; }
.pill-btn.active { background: var(--vp-c-brand-1); color: #fff; border-color: var(--vp-c-brand-1); }
.ctrl-group label { font-size: 12.5px; font-weight: 600; color: var(--vp-c-text-1); }
.ctrl-group .val { color: var(--vp-c-brand-1); font-family: monospace; }
.oscillo-panel { display: grid; grid-template-columns: 180px 1fr; gap: 20px; align-items: center; }
.meter-view { text-align: center; padding: 16px; border-radius: 8px; background: var(--vp-c-bg); border: 1px solid var(--vp-c-divider); }
