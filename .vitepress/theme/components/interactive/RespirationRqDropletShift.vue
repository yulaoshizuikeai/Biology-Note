<template>
  <div class="interactive-card">
    <div class="card-header">
      ...
      <p class="card-desc">
        切换底物与氧气工况，动态观察装置一（含 NaOH）与装置二（含清水）红墨水滴移动方向。
      </p>
    </div>
    ...
    <div class="control-grid">
      <div class="control-item">
        ...
        <div class="pill-group">
          <button
            :class="['pill-btn', { active: respType === 'aerobic_glucose' }]"
            @click="respType = 'aerobic_glucose'"
          >
            有氧分解葡萄糖
          </button>
          <button
            :class="['pill-btn', { active: respType === 'aerobic_lipid' }]"
            @click="respType = 'aerobic_lipid'"
          >
            有氧分解脂肪 (富氢)
          </button>
          <button
            :class="['pill-btn', { active: respType === 'anaerobic_alcohol' }]"
            @click="respType = 'anaerobic_alcohol'"
          >
            酵母菌酒精发酵
          </button>
          <button
            :class="['pill-btn', { active: respType === 'anaerobic_lactate' }]"
            @click="respType = 'anaerobic_lactate'"
          >
            乳酸菌乳酸发酵
          </button>
        </div>
    ...
    <div class="apparatus-grid">
      <div class="apparatus-box">
        ...
        <div class="app-status">
          液滴状态：<strong>{{ drop1Desc }}</strong>
        </div>
        <div class="app-hint">
          NaOH 吸收全部 CO₂，故装置一气压变化仅由 <strong>O₂ 消耗量</strong> 决定！
        </div>
      </div>
      ...
      <div class="apparatus-box">
        ...
        <div class="pipe-view">
          ...
          <div class="pipe-labels">
            <span>左移 (耗O₂ &gt; 产CO₂)</span><span>不动</span><span>右移 (产CO₂ &gt; 耗O₂)</span>
          </div>
        </div>
        <div class="app-status">
          液滴状态：<strong>{{ drop2Desc }}</strong>
        </div>
        <div class="app-hint">
          装置二气压变化取决于 <strong>CO₂ 释放量与 O₂ 消耗量的差值</strong>！
        </div>
      </div>
    </div>
    ...
    <div class="rq-summary">
      <div class="rq-val">
        呼吸熵 RQ 判定：<strong>{{ rqResult }}</strong>
      </div>
      <div class="rq-rule">
        ★
        物理对照校正原则：必须设置将活材料替换为<strong>等体积死材料（如煮沸杀死的种子）</strong>的对照组以消除物理温压波动误差。
      </div>
    </div>
  </div>
</template>
...
const drop1Desc = computed(() => {
  if (respType.value === "aerobic_glucose" || respType.value === "aerobic_lipid")
    return "向左移动（消耗 O₂）";
  return "保持不动（不消耗 O₂）";
});
