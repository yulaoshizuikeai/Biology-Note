<template>
  <div class="interactive-card">
    <div class="card-header">
      ...
      <p class="card-desc">
        开关光照与 CO₂ 供应，动态观察 ATP、[H] 与 C₃、C₅
        库容波形，验证“断光升三降五，断碳升五降三”。
      </p>
    </div>
    ...
    <div class="gauges-grid">
      <div class="gauge-card">
        ...
        <div class="gauge-bar-bg">
          <div class="gauge-bar bg-amber" :style="{ width: atpHLevel + '%' }"></div>
        </div>
        <div class="gauge-meta">
          <span>库容水平</span><strong>{{ atpHLevel }}%</strong>
        </div>
      </div>
      <div class="gauge-card">
        <div class="gauge-title">C₃ 含量 (三碳酸还原物)</div>
        <div class="gauge-bar-bg">
          <div class="gauge-bar bg-emerald" :style="{ width: c3Level + '%' }"></div>
        </div>
        <div class="gauge-meta">
          <span>相对含量</span><strong>{{ c3Level }}%</strong>
        </div>
      </div>
      <div class="gauge-card">
        <div class="gauge-title">C₅ 含量 (五碳核酮糖接受体)</div>
        <div class="gauge-bar-bg">
          <div class="gauge-bar bg-blue" :style="{ width: c5Level + '%' }"></div>
        </div>
        <div class="gauge-meta">
          <span>相对含量</span><strong>{{ c5Level }}%</strong>
        </div>
      </div>
      <div class="gauge-card">
        <div class="gauge-title">(CH₂O) 净合成速率</div>
        <div class="gauge-bar-bg">
          <div class="gauge-bar bg-purple" :style="{ width: sugarRate + '%' }"></div>
        </div>
        <div class="gauge-meta">
          <span>输出效率</span><strong>{{ sugarRate }}%</strong>
        </div>
      </div>
    </div>
