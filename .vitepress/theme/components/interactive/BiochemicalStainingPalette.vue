<template>
  <div class="interactive-card">
    <div class="card-header">
      ...
      <p class="card-desc">点击选择待测物质与检测试剂，即时模拟化学显色反应、反应条件与考场排雷要诀。</p>
    </div>

    <div class="reagent-pills">
      <button v-for="r in reagents" :key="r.id" :class="['r-btn', { active: currentReagent === r.id }]" @click="currentReagent = r.id">
        {{ r.name }}
      </button>
    </div>
...
const reagentData = {
  ...
  biuret: {
    ...
    warning: '必须【先后加入】：先加 A 液 1mL 营造碱性环境，再滴入 B 液 4 滴；B 液严禁过量（否则铜离子蓝色掩盖紫色）。'
  },
  sudan: {
    substance: '脂肪 (甘油三酯)',
    ...
    hasPrecipitate: false,
    warning: '花生子叶染色后必须滴加【体积分数 50% 的酒精溶液洗去浮色】，因为苏丹染料易溶于有机溶剂酒精。'
  },
  alcohol: {
    substance: '酒精 (乙醇)',
    reagentName: '重铬酸钾溶液',
  ...
  diphenylamine: {
    ...
    colorName: '蓝色',
    hasPrecipitate: false,
    warning: '粗提取 DNA 时利用在 0.14 mol/L NaCl 中溶解度最低析出，并在 95% 预冷酒精中沉淀；二苯胺必须沸水浴变蓝。'
  }
};
...
.reagent-pills { display: flex; gap: 8px; margin: 16px 0; flex-wrap: wrap; }
.r-btn { padding: 6px 12px; border-radius: 6px; font-size: 12px; font-weight: 600; border: 1px solid var(--vp-c-divider); background: var(--vp-c-bg); color: var(--vp-c-text-2); cursor: pointer; transition: all 0.2s; }
.r-btn.active { background: var(--vp-c-brand-1); color: #fff; border-color: var(--vp-c-brand-1); }
.test-tube-stage { display: grid; grid-template-columns: 140px 1fr; gap: 20px; align-items: center; }
.tube-container { text-align: center; }
