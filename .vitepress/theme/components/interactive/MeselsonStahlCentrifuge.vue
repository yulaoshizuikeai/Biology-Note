<template>
  <div class="interactive-card">
    <div class="card-header">
      ...
      <p class="card-desc">
        输入 DNA 复制代数 n，实时查看氯化铯超速离心管中轻带、中带、重带分层与分子守恒。
      </p>
    </div>

    <div class="control-row">
      <label
        >转移到 ¹⁴N 培养基中连续复制代数 (n): <span class="val">{{ n }}</span> 代</label
      >
    ...
    <div class="centrifuge-display">
      ...
      <div class="calc-metrics">
        ...
        <div class="metric-card">
          ...
          <div class="m-val text-amber">{{ parentStrandDnaText }}</div>
        </div>
        <div class="metric-card">
        ...
        <div class="metric-card">
          ...
          <div class="m-val text-purple">{{ parentStrandPercent }}%</div>
        </div>
      </div>
    </div>
  </div>
...
const parentStrandDnaText = computed(() => (n.value === 0 ? "1 个 (亲代全含)" : "2 个 (恒定为 2)"));
...
const lightPercent = computed(() =>
  n.value === 0 ? 0 : Math.round(((totalDna.value - 2) / totalDna.value) * 100),
);
const parentStrandPercent = computed(() =>
  n.value === 0 ? 100 : Math.round((2 / totalDna.value) * 100),
);
...
const lightOpacity = computed(() =>
  n.value === 0 ? 0.05 : Math.min(1, Math.max(0.2, (totalDna.value - 2) / totalDna.value)),
);
</script>
