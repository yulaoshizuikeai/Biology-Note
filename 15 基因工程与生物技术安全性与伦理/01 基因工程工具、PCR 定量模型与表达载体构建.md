...
date: 2026-03-09
description: "精讲限制酶与 DNA 连接酶作用位点、PCR 引物方向与轮数扩增定量模型，以及双酶切防自连、启动子与终止子驱动的表达载体构建机理。"
---
...
- **纯目的基因（两端均受引物限定的等长片段）**：
  - 第 1 轮循环：以原始 DNA 为模板，产物均为一端定长（引物端）、另一端不定长的长单链 DNA；
  - 第 2 轮循环：以第 1 轮产物为模板，**首次合成两端均受引物限定的目标单链 DNA**（但尚未出现等长双链）；
  - **第 3 轮循环：首次出现双链两端均等长、符合目标长度的目的基因双链片段！**
...
3. **质粒破坏标记基因用于筛选（插入失活法）**：若目的基因插入到四环素抗性基因内部，则该抗性基因被**破坏失活**（**插入失活法**，Insertional Inactivation），此时重组质粒具有氨苄青霉素抗性而不具有四环素抗性，可据此在不同抗生素平板上筛选。
   :::
